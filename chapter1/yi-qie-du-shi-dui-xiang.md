...
CPP通过几个关键字引入了命名空间的概念 . 而Java为了给一个类库生成不会与其他名字混淆的名字 , 希望开发者反过来使用自己的域名 , 句点代表子目录的划分\(Java 2开始都使用小写\) . 这种机制意味着所有的文件都能够自动存活于它们自己的命名空间内 , 而且同一个文件内的每个类都有唯一的标识符 .
...
类名首字母大写 , 类名由几个单词构成 , 驼峰式写法 . 其他类中的方法 , 字段以及对象引用名称等 , 都是驼峰式写法 , 只是首字母是小写的 .
